...
async function getinfo(msg, D, color, command, ID) {
    ...
    $("#chapter_list > ul > div > div").eq(0).find("li").each(function(){
        let chapter = $(this).find("div.epsleft > div.lchx > a").text()
        let chapterID = $(this).find("div.epsleft > div.lchx > a").attr("href")
        let date = $(this).find("div.epsleft > div.date").text()
        chapterlist.push({chapter,chapterID,date})
    })
    ...
    let embed = new D.MessageEmbed();
        ...
        if(info[0]){ embed.addField(info[0].key, info[0].value)}
        if(info[1]){ embed.addField(info[1].key, info[1].value)}
        if(info[2]){ embed.addField(info[2].key, info[2].value)}
